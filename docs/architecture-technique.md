...
- **Jauges de feedback** : affichage de la recette courante et de sa progression, score de la requête décomposé à la validation ("+12 synergie, +8 objectif" — exigence de lisibilité de la section 4 ; pas de ligne timing au MVP, ce score optionnel étant hors périmètre du premier prototype, cf. roadmap), score cumulé du set. Un indicateur de beat (pulsation visuelle du transport) sera ajouté si/quand le bonus de timing est implémenté — post-MVP.
...
- **`data/scoring.json`** — tous les paramètres de la Proposition 1 (poids des paires, bonus, paliers d'étoiles), la table des contradictions de Genre, et le bonus de contraste proposé (`contrast_pair_bonus`, à valider).
